...
import {
  ...
  slideVariants,
  TOTAL_STEPS,
} from '../../hooks/useNeighbourVouchingFlow'
...
export function NeighbourVouchingFlow(): React.JSX.Element {
  ...
  return (
    <div className="font-dmsans relative flex h-full w-full flex-col overflow-hidden bg-white">
      ...
      <AnimatePresence>
        {topToast && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -20 }}
            className="bg-actionDark absolute top-4 right-6 left-6 z-50 rounded-xl px-4 py-3 text-center text-[13px] font-medium text-white shadow-lg"
          >
            {topToast}
          </motion.div>
        )}
      ...
      <div className="hide-scrollbar flex flex-1 flex-col overflow-y-auto">
        <AnimatePresence mode="wait" custom={direction}>
          ...
          >
            {/* 1. INTRO STEP */}
            {state.currentStep === 'INTRO' && (
              <IntroStep onStart={() => goToStep('DETAILS', 1)} onBack={handleBack} />
            )}
