...
export function useAuthForm() {
  ...
  const setField = useCallback(
    <K extends keyof Omit<AuthFormState, 'errors'>>(field: K, value: string) => {
      setState((prev) => ({
        ...prev,
        [field]: value,
  ...
  const validatePhone = useCallback((): boolean => {
    ...
    setState((prev) => ({
      ...prev,
      errors: { ...prev.errors, phone: valid ? undefined : 'Enter a valid phone number' },
    }))
    return valid
  }, [state.phone])

  const validateOtp = useCallback((): boolean => {
    const valid = state.otp.trim().length === 6
    setState((prev) => ({
      ...prev,
      errors: { ...prev.errors, otp: valid ? undefined : 'Enter the 6-digit code' },
    }))
    return valid
  }, [state.otp])
